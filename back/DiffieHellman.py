...
class DiffieHellman(object):
	...
	def __init__(self, generator=2, group=17, keyLength=540):
		"""
		Generate the public and private keys.生成公钥和私钥。
		"""
		min_keyLength = 180 #密钥最少180位
		default_keyLength = 540

		default_generator = 2
		valid_generators = [ 2, 3, 5, 7 ]

		# Sanity check fors generator and keyLength 检查生成器和密钥长度
		...
		if(keyLength < min_keyLength):
			...
			self.keyLength = keyLength #密钥最少180位其他的情况与传入参数一样

		self.prime = self.getPrime(group)

		self.privateKey = self.genPrivateKey(keyLength) #生成keyLenth位的私钥
		self.publicKey = self.genPublicKey()

	def getPrime(self, group=17):
		"""
		Given a group number, return a prime.给定一个组号，返回一个素数。
		"""
		default_group = 17
	...
	def genRandom(self, bits):
		"""
		Generate a random number with the specified number of bits 生成具有指定位数的随机数
		"""
		_rand = 0
		_bytes = bits // 8 + 8
	...
	def genPrivateKey(self, bits):
		"""
		Generate a private key using a secure random number generator.使用安全随机数生成器生成私钥。
		"""
		return self.genRandom(bits)

	def genPublicKey(self):
		"""
		Generate a public key X with g**x % p.用g** X % p生成一个公钥X。pow(x,y,z)
	...
	def checkPublicKey(self, otherKey):
		"""
		Check the other party's public key to make sure it's valid.检查对方的公钥以确保它是有效的。
		Since a safe prime is used, verify that the Legendre symbol == 1 由于使用了安全素数，请验证勒让德符号== 1
		"""
	...
	def genSecret(self, privateKey, otherKey):
		...
		private key to generate a shared secret.检查以确保公钥有效，然后将其与生成共享密钥的私钥。
		"""
	...
	def genKey(self, otherKey):
		"""
		Derive the shared secret, then hash it to obtain the shared key.派生共享密钥，然后哈希它以获得共享密钥。
		"""
		self.sharedSecret = self.genSecret(self.privateKey, otherKey)

		# Convert the shared secret (int) to an array of bytes in network order将共享机密(int)转换为按网络顺序排列的字节数组
		# Otherwise hashlib can't hash it.否则，hashlib无法哈希它。
		try:
	...
	def getKey(self):
		"""
		Return the shared secret key返回共享密钥
		"""
		return self.key

	def showParams(self):
		"""
		Show the parameters of the Diffie Hellman agreement.显示DH的参数。
		"""
		print("Parameters:")
	...
	def showResults(self):
		"""
		Show the results of a Diffie-Hellman exchange.显示一个DH交换的结果。
		"""
		print("Results:")
